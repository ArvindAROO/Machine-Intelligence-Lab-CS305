...
class Tensor:
    ...
    def grad_add(self, gradients=None):
        ...
        """
        # TODO
        tensor_a = self.history[1]
        tensor_b = self.history[2]
        tensor_a.grad=np.zeros_like(tensor_a.arr)
        tensor_b.grad=np.zeros_like(tensor_b.arr)
        if tensor_b.requires_grad == True: 
            tensor_b.grad += np.ones_like(tensor_b.arr)
        if tensor_a.requires_grad == True: 
            tensor_a.grad += np.ones_like(tensor_a.arr)
        
        if gradients is not None:
            if tensor_b.requires_grad == True: 
                tensor_b.grad = np.multiply(np.ones_like(tensor_b.arr), gradients)
            if tensor_a.requires_grad == True: 
                tensor_a.grad = np.multiply(np.ones_like(tensor_a.arr), gradients)
            
        return (tensor_a.grad, tensor_b.grad)
    ...
    def grad_matmul(self, gradients=None):
        ...
        if gradients is not None:
            if tensor_b.requires_grad == True:
                tensor_b.grad += np.multiply(np.matmul(gradients, np.ones_like(tensor_b.arr)), tensor_a.arr.transpose())
            if tensor_a.requires_grad == True:
                tensor_a.grad += np.multiply(np.matmul(np.ones_like(tensor_a.arr), tensor_b.arr.transpose()), gradients)
            
        else:
            
            if tensor_b.requires_grad == True:
                tensor_b.grad += np.matmul(np.ones_like(tensor_b.arr), tensor_a.arr).transpose()
            if tensor_a.requires_grad == True:
                tensor_a.grad += np.matmul(np.ones_like(tensor_a.arr), tensor_b.arr.transpose())
    ...
    def backward(self, gradients=None):
        ...
        if self.history[0] == 'matmul':
            resultOfNextGrad = self.grad_matmul(gradients)
            if self.history[1] is not None:
                self.history[1].backward(resultOfNextGrad[0])
            if self.history[2] is not None:
                self.history[2].backward(resultOfNextGrad[1])
        
        elif self.history[0] == 'add':
            resultOfNextGrad = self.grad_add(gradients)
            if self.history[1] is not None:
                self.history[1].backward(resultOfNextGrad[0])
            if self.history[2] is not None:
                self.history[2].backward(resultOfNextGrad[1])
